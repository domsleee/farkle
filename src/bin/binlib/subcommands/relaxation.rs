...
pub fn run_relaxation(_args: &MyArgs, relaxation_args: &RelaxationArgs) {
    ...
    let scores = if relaxation_args.scores.is_empty() {
        vec![0, 0]
    } else {
        relaxation_args.scores.to_owned()
    };
    ...
    for held_score in (0..=10000).step_by(50) {
        for score1 in (scores[0]..=10000).step_by(50) {
            for score2 in (scores[1]..=10000).step_by(50) {
                ...
                let cache_key = get_cache_key::<2>(held_score, dice_left, &[score1, score2]);
    ...
    for it in 1..=i32::MAX {
        ...
        for (held_score, dice_left, scores) in &all_keys {
            let cache_key = get_cache_key(*held_score, *dice_left, scores);
            cache_prev_run.insert(
                cache_key,
                solver.decide_action_ext(*held_score, *dice_left, *scores).0,
            );
        }
        delta = 0.00f64;
        for (held_score, dice_left, scores) in &all_keys {
            let cache_key = get_cache_key(*held_score, *dice_left, scores);
            let this_delta = f64::abs(
                cache_prev_run.get(&cache_key).unwrap()
                    - prev_cache_prev_run.get(&cache_key).unwrap(),
            );
